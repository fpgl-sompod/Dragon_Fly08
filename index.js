    ...
    .defines(function () {
        ButtonCard = EntityButton.extend({
            ...
            drawImage: function () {
                ...
                b.translate(this.pos.x + 0.5 * this.image.width * this.scale.x + 130, this.pos.y + 0.5 * this.image.height * this.scale.y - this.adjustY);
...
ig.module("game.entities.GameOver")
    ...
    .defines(function () {
        ...
        GameOverEntity = ig.Entity.extend({
            ...
            init: function (a, c, d) {
                ...
                //this.buttonFeed = ig.game.spawnEntity(ButtonCard, 0, 0, { image: this.imageInstall, cbClicked: this.control.buttonFeedClicked.bind(this.control), visible: !0, isAnimating: !0, factorY: 0.5, scaleFactor: 0.5 });
            },
            update: function () {
                // Update all entities and backgroundMaps
                this.parent();
                this.control.updatePointer();
                this.control.detectPointer();
                //this.InstallButtonClick();
            },

            InstallButtonClick: function () {
                if (MJS.view.viewport.orientation.portrait) {
                    if (ig.input.mouse.x >= 108 && ig.input.mouse.x <= 235) {
                        if (ig.input.mouse.y >= 390 && ig.input.mouse.y <= 464) {
                            ig.input.mouse.x = 0;
                            ig.input.mouse.y = 0;
                            this.control.buttonFeedClicked();
                        }
                    }
                }
                else {
                    if (ig.input.mouse.x >= 189 && ig.input.mouse.x <= 296) {
                        if (ig.input.mouse.y >= 245 && ig.input.mouse.y <= 318) {
                            ig.input.mouse.x = 0;
                            ig.input.mouse.y = 0;
                            this.control.buttonFeedClicked();
                        }
            ...
            DrawCounter: function () {
                if (MJS.view.viewport.orientation.portrait) {
                    ...
                    b.translate(280, 480);
                    b.lineWidth = 3;
                    b.fillText("Score : " + this.control.score, 10, 0);
                    b.restore();
            ...
            draw: function () {
                ...
                if (MJS.view.viewport.orientation.portrait) {
                    ...
                    if (this.installControl == 0) {
                        if (this.installAnimY >= 540) {
                            this.installAnimY -= 0.25;
                        } else {
                            this.installControl = 1;
                        }
                    } else {
                        if (this.installAnimY <= 560) {
                            this.installAnimY += 0.25;
                        } else {
                            this.installControl = 0;
                        }
                    }
                    b.translate(60, this.installAnimY);
                    
                    this.imageInstall.draw(0, 0);
                    b.restore();
                    ...
                    if (this.installControl == 0) {
                        if (this.installAnimYLand >= 285) {
                            this.installAnimYLand -= 0.25;
                        } else {
                            this.installControl = 1;
                        }
                    } else {
                        if (this.installAnimYLand <= 295) {
                            this.installAnimYLand += 0.25;
                        } else {
                            this.installControl = 0;
                        }
                    }
                    b.translate(215, this.installAnimYLand);
...
ig.baked = !0;
ig.module("game.entities.bigDragon_anim")
    .requires("impact.entity")
    .defines(function () {
        BigDragonAnim = ig.Entity.extend({
            imageSheet: new ig.AnimationSheet(imagePath.biganim, 364, 385),
            imageCoin: new ig.Image(imagePath.coin),
            zIndex: 1000,
            animBabyDragonVar: null,
            posOriginX: null,
            posOriginY: null,
            posCurrent: null,
            posTarget: null,
            ...
            init: function (b, c, d) {
                this.parent(b, c, d);
                d.posCurrent && ((this.posCurrent = d.posCurrent), (this.posOriginX = this.posCurrent.x), (this.posOriginY = this.posCurrent.y));
                d.posTarget && (this.posTarget = d.posTarget);
                d.control && (this.control = d.control);
                this.animBigDragonVar = new ig.Animation(this.imageSheet, 0.1, [0, 1, 2, 3, 4, 5], 0);

                this.posCoin = {x: 800, y: 600};
                this.posCoin1 = {x: 1400, y: 800};
                this.posCoin2 = {x: 1500, y: 500};
            },
            update: function () {
                this.parent();
                if (this.pauseGame == 0) {
                    if (this.isgameover == 0) {
                        this.animBigDragonVar.update();
                        this.startCoinTween();
                        this.startCoinTween1();
                        this.startCoinTween2();
                        if (this.leftToRight == 0) {
                            this.leftToRight = 1;
                            this.startTween();

                            this.temppos = this.posCurrent.x;
                        }
                    } else {
                        //this.tween.stopTween();
                    }
                }
            ...
            draw: function () {
                ...
                this.parent();
                var area = ig.system.context;
                area.save();
                area.translate(this.posCurrent.x, this.posCurrent.y);
                if (MJS.view.viewport.orientation.portrait) {
                    area.scale(0.5, 0.5);
                } else {
                    area.scale(0.4, 0.4);
                ...
                if (this.isgameover == 0) {
                    this.animBigDragonVar.draw(- 0.5 * 364, - 0.5 * 385);
            ...
            drawCoin: function () {
                var dcoin = ig.system.context;
                dcoin.save();
                dcoin.translate(this.posCoin.x, this.posCoin.y);
                dcoin.scale(0.06 , 0.06);
                this.imageCoin.draw(-0.5 * this.imageCoin.width, -0.5 * this.imageCoin.height);
                dcoin.restore();

                var dcoin = ig.system.context;
                dcoin.save();
                dcoin.translate(this.posCoin1.x, this.posCoin1.y);
                dcoin.scale(0.06 , 0.06);
                this.imageCoin.draw(-0.5 * this.imageCoin.width, -0.5 * this.imageCoin.height);
                dcoin.restore();
                ...
                var dcoin = ig.system.context;
                dcoin.save();
                dcoin.translate(this.posCoin2.x, this.posCoin2.y);
                dcoin.scale(0.06 , 0.06);
                this.imageCoin.draw(-0.5 * this.imageCoin.width, -0.5 * this.imageCoin.height);
                dcoin.restore();
            },
            ...
            startTween: function () {
                ...
                this.tween({ posCurrent: { x: -100, y: this.posCurrent.y } }, 1.6, {
                    onComplete: function () {
                        if (MJS.view.viewport.orientation.portrait) {
                            this.posCurrent = { x: 800, y: Math.floor(Math.random() * 660) + 60 };
                        } else {
                            this.posCurrent = { x: 1100, y: Math.floor(Math.random() * 300) + 60 };
                        }
            ...
            startCoinTween: function () {
                if (this.posCoin.x > -100) {
                    
                    this.posCoin = { x: this.posCoin.x - 3, y: this.posCoin.y }
                } else {
                        if (MJS.view.viewport.orientation.portrait) {
                            this.posCoin = { x: 800, y: Math.floor(Math.random() * 660) + 60 };
                        } else {
                            this.posCoin = { x: 1100, y: Math.floor(Math.random() * 300) + 60 };
                        }
                }
                //this.tween({ posCoin: { x: -100, y: this.posCoin.y } }, 1.6, {
                //    onComplete: function () {
                //        if (MJS.view.viewport.orientation.portrait) {
                //            this.posCoin = { x: 800, y: Math.floor(Math.random() * 660) + 60 };
                //        } else {
            ...
            startCoinTween1: function () {
                if (this.posCoin1.x > -100) {
                    this.posCoin1 = { x: this.posCoin1.x - 3, y: this.posCoin1.y }
                } else {
                    if (MJS.view.viewport.orientation.portrait) {
                        this.posCoin1 = { x: 1100, y: Math.floor(Math.random() * 660) + 60 };
                    } else {
                        this.posCoin1 = { x: 1400, y: Math.floor(Math.random() * 300) + 60 };
                    }
                }
                //this.tween({ posCoin1: { x: -100, y: this.posCoin1.y } }, 1.6, {
                //    onComplete: function () {
                //        if (MJS.view.viewport.orientation.portrait) {
                //            this.posCoin1 = { x: 800, y: Math.floor(Math.random() * 660) + 60 };
                //        } else {
                //            this.posCoin1 = { x: 1400, y: Math.floor(Math.random() * 300) + 60 };
                //        }
            ...
            startCoinTween2: function () {
                if (this.posCoin2.x > -100) {
                    this.posCoin2 = { x: this.posCoin2.x - 3, y: this.posCoin2.y }
                } else {
                    if (MJS.view.viewport.orientation.portrait) {
                        this.posCoin2 = { x: 1400, y: Math.floor(Math.random() * 660) + 60 };
                    } else {
                        this.posCoin2 = { x: 1700, y: Math.floor(Math.random() * 300) + 60 };
                    }
                }
                //this.tween({ posCoin2: { x: -100, y: this.posCoin2.y } }, 1.6, {
                //    onComplete: function () {
                //        if (MJS.view.viewport.orientation.portrait) {
                //            this.posCoin2 = { x: 800, y: Math.floor(Math.random() * 660) + 60 };
                //        } else {
                //            this.posCoin2 = { x: 1600, y: Math.floor(Math.random() * 300) + 60 };
                //        }
...
ig.baked = !0;
ig.module("game.entities.babyDragon_anim")
    .requires(
        "impact.entity"
)
    .defines(function () {
        BabyDragonAnim = ig.Entity.extend({
            imageSheet: new ig.AnimationSheet(imagePath.babyanim, 369, 389),
            zIndex: 1000,
            animBabyDragonVar: null,
            posOriginX: null,
            posOriginY: null,
            ...
            init: function (b, c, d) {
                ...
                d.posTarget && (this.posTarget = d.posTarget);
                d.control && (this.control = d.control);
                this.animBabyDragonVar = new ig.Animation(this.imageSheet, 0.1, [0, 1, 2, 3, 4, 5], 0);
            },
            update: function () {
                this.parent();
                if (this.pauseGame == 0) {
                    this.animBabyDragonVar.update();
                }
            ...
            draw: function () {
                ...
                this.animBabyDragonVar.draw(- 0.5 * 369, - 0.5 * 389);
                area.restore();
                if (this.pauseGame == 0) {
                    if (this.isgameover == 0) {
                        if (MJS.view.viewport.orientation.portrait) {

                            if (this.posCurrent.y < 750) {
                                var tempY = this.posCurrent.y + 4;
                                this.posCurrent = { x: this.posCurrent.x, y: tempY };
                            } else {
                                this.isgameover = 1;
                            }
                        } else {

                            if (this.posCurrent.y < 380) {
                                var tempY = this.posCurrent.y + 3;
                                this.posCurrent = { x: this.posCurrent.x, y: tempY };
                            } else {
                                this.isgameover = 1;
                            }
            ...
            tapcount: 0,
            JumpBabyDragon: function (jumpspeed) {
                if (this.pauseGame == 0) {
                    if (this.posCurrent.y> 60) {
                        this.posCurrent = { x: this.posCurrent.x, y: this.posCurrent.y - jumpspeed };
                    }
            ...
            startTween: function () {
                this.tween({ posCurrent: { x: this.posCurrent.x, y: 300 } }, 0.5, {
                    onComplete: function () {
...
ig.module("game.entities.game-control")
    ...
    .defines(function () {
        EntityGameControl = ig.Entity.extend({
            ...
            imageSKYBG1: new ig.Image(imagePath.skybg),
            imageSKYBG2: new ig.Image(imagePath.skybg),
            imageLogo: new ig.Image(imagePath.imageLogo),
            imageInstall: new ig.Image(imagePath.installNow),
            imageBlackLayer: new ig.Image(imagePath.blackLayer),
            imagePnff: new ig.Image(imagePath.pnff),
            imageHand: new ig.Image(imagePath.hand),
            ...
            init: function (b, c, d) {
                ...
                if (MJS.view.viewport.orientation.portrait) {
                    this.babyAnimEntity = ig.game.spawnEntity(BabyDragonAnim, 0, 0, {
                        posCurrent: { x: 100, y: 200 },
                        posTarget: { x: 200, y: 200 }, control: this
                    });
                } else {
                    this.babyAnimEntity = ig.game.spawnEntity(BabyDragonAnim, 0, 0, {
                        posCurrent: { x: 200, y: 200 },
                        posTarget: { x: 200, y: 200 }, control: this
                    });
                }

                //this.babyAnimEntity.posCurrent = {x: 300 * -1, y: 200};

                this.bigAnimEntity = ig.game.spawnEntity(BigDragonAnim, 0, 0, {
                    posCurrent: { x: 700, y: 200 },
                    posTarget: { x: 200, y: 200 }, control: this
                });

                setTimeout(() => {
                    this.isdrawTutorial = 1;
                    this.gamestart();
            ...
            gamestart: function () {
                this.babyAnimEntity.pauseGame = 0;
                this.bigAnimEntity.pauseGame = 0;
                this.gamepause = 0;
            },
            initTween: function () {
            ...
            draw: function () {
                this.parent();
                this.DrawBg();
                if (this.isdrawTutorial == 0) {
                    this.drawTutorialHand();
                } else if (this.isdrawTutorial == 1) {
                    
                    this.ScoreCount();
                    this.isdrawTutorial = 2;

                }
                if (this.isdrawTutorial == 2) {
                    this.DrawCounter();
                }
                if (this.isGameOver == 1) {
                    this.isGameOver = 2;
                    this.gameOverEntity = ig.game.spawnEntity(GameOverEntity, 0, 0, { control: this });
                        //this.drawGameOver();
                }
            ...
            DrawCounter: function () {
                if (MJS.view.viewport.orientation.portrait) {
                    ...
                    b.translate(420, 50);
                    b.lineWidth = 3;
                    b.fillText("Score : " + this.score, 10, 0);
                    b.restore();
            ...
            bgPortX1: 2000,
            bgPortX2: 3400,
            DrawBg: function () { // this function for draw Background of games
                if (MJS.view.viewport.orientation.portrait) {
                    this.EndlessBg();
                    var area = ig.system.context;
                    area.save();
                    area.translate(this.bgPortX, ig.system.height / 2);
                    area.scale(0.35, 0.5);
                    this.imageSKYBG.draw(-this.imageSKYBG.width / 2, -this.imageSKYBG.height / 2);
                    area.restore();
                    var area = ig.system.context;
                    area.save();
                    area.translate(this.bgPortX1, ig.system.height / 2);
                    area.scale(0.35, 0.5);
                    this.imageSKYBG1.draw(-this.imageSKYBG1.width / 2, -this.imageSKYBG1.height / 2);
                    area.restore();
                    var area = ig.system.context;
                    area.save();
                    area.translate(this.bgPortX2, ig.system.height / 2);
                    area.scale(0.35, 0.5);
                    this.imageSKYBG2.draw(-this.imageSKYBG2.width / 2, -this.imageSKYBG2.height / 2);
                    area.restore();
                }
                else {
                    this.EndlessBg();
                    var area = ig.system.context;
                    area.save();
                    area.translate(this.bgLandscapeX, ig.system.height / 2);
                    area.scale(0.3, 0.3);
                    this.imageSKYBG.draw(-this.imageSKYBG.width / 2, -this.imageSKYBG.height / 2);
                    area.restore();
                    var area = ig.system.context;
                    area.save();
                    area.translate(this.bgLandscapeX1, ig.system.height / 2);
                    area.scale(0.3, 0.3);
                    this.imageSKYBG1.draw(-this.imageSKYBG1.width / 2, -this.imageSKYBG1.height / 2);
                    area.restore();
                    var area = ig.system.context;
                    area.save();
                    area.translate(this.bgLandscapeX2, ig.system.height / 2);
                    area.scale(0.3, 0.3);
                    this.imageSKYBG2.draw(-this.imageSKYBG2.width / 2, -this.imageSKYBG2.height / 2);
                    area.restore();
                }
            ...
            time1land: 4,
            time2land: 6,
            EndlessBg: function () { //this is for moving background
                //those for portrite mode
                ...
                if (this.bgPortX1 > -700) {
                    this.bgPortX1 -= 5;
                }
                else {
                    this.bgPortX1 = 3400;
                }
                if (this.bgPortX2 > -700) {
                    this.bgPortX2 -= 5;
                } else {
                    this.bgPortX2 = 3400;
                }

                // those code for landscape

                if (this.bgLandscapeX > - 600) {
                    this.bgLandscapeX -= 5;
                } else {
                    this.bgLandscapeX = 2990;
                }
                if (this.bgLandscapeX1 > -600) {
                    this.bgLandscapeX1 -= 5;
                } else {
                    this.bgLandscapeX1 = 2990;
                }
                if (this.bgLandscapeX2 > -600) {
                    this.bgLandscapeX2 -= 5;
                } else {
                    this.bgLandscapeX2 = 2990;
                }
            ...
            update: function () { //master update function 
                if (this.gamepause == 0) {
                    if (this.isGameOver == 0) {
                        ...
                        if (this.babyAnimEntity.isgameover == 1) {
                            this.isGameOver = 1;
                            this.babyAnimEntity.IsGameOver();
                            this.bigAnimEntity.IsGameOver();
                        }
                    }
            ...
            CollisionDetection: function () {
                var babycollisionX = this.babyAnimEntity.posCurrent.x - (370 * 0.3 / 2);
                var babycollisionX1 = this.babyAnimEntity.posCurrent.x + (370 * 0.3 / 2);
                var babycollisionY = this.babyAnimEntity.posCurrent.y - (389 * 0.3 / 4);
                var babycollisionY1 = this.babyAnimEntity.posCurrent.y + (389 * 0.3 / 4);
                var bigcollisionX = this.bigAnimEntity.posCurrent.x - (364 * 0.5 / 2);
                var bigcollisionX1 = this.bigAnimEntity.posCurrent.x + (364 * 0.5 / 2);
                var bigcollisionY = this.bigAnimEntity.posCurrent.y - (385 * 0.5 / 2);
                var bigcollisionY1 = this.bigAnimEntity.posCurrent.y + (385 * 0.5 / 2);

                var CoincollisionX = this.bigAnimEntity.posCoin.x - (this.bigAnimEntity.imageCoin.width * 0.5 / 2);
                var CoincollisionX1 = this.bigAnimEntity.posCoin.x + (this.bigAnimEntity.imageCoin.width * 0.5 / 2);
                var CoincollisionY = this.bigAnimEntity.posCoin.y - (this.bigAnimEntity.imageCoin.height * 0.5 / 2);
                var CoincollisionY1 = this.bigAnimEntity.posCoin.y + (this.bigAnimEntity.imageCoin.height * 0.5 / 2);

                var Coin1collisionX = this.bigAnimEntity.posCoin1.x - (this.bigAnimEntity.imageCoin.width * 0.5 / 2);
                var Coin1collisionX1 = this.bigAnimEntity.posCoin1.x + (this.bigAnimEntity.imageCoin.width * 0.5 / 2);
                var Coin1collisionY = this.bigAnimEntity.posCoin1.y - (this.bigAnimEntity.imageCoin.height * 0.5 / 2);
                var Coin1collisionY1 = this.bigAnimEntity.posCoin1.y + (this.bigAnimEntity.imageCoin.height * 0.5 / 2);

                var Coin2collisionX = this.bigAnimEntity.posCoin2.x - (this.bigAnimEntity.imageCoin.width * 0.5 / 2);
                var Coin2collisionX1 = this.bigAnimEntity.posCoin2.x + (this.bigAnimEntity.imageCoin.width * 0.5 / 2);
                var Coin2collisionY = this.bigAnimEntity.posCoin2.y - (this.bigAnimEntity.imageCoin.height * 0.5 / 2);
                var Coin2collisionY1 = this.bigAnimEntity.posCoin2.y + (this.bigAnimEntity.imageCoin.height * 0.5 / 2);
                //console.log("babay pos more : " + collisionX);
                if (babycollisionX >= bigcollisionX && babycollisionX <= bigcollisionX1) {

                    if (this.babyAnimEntity.posCurrent.y >= bigcollisionY && this.babyAnimEntity.posCurrent.y <= bigcollisionY1) {
                        this.isGameOver = 1;
                        this.babyAnimEntity.IsGameOver();
                        this.bigAnimEntity.IsGameOver();
                    }
                ...
                if (babycollisionX >= CoincollisionX && babycollisionX <= CoincollisionX1) {

                    if (this.babyAnimEntity.posCurrent.y >= CoincollisionY && this.babyAnimEntity.posCurrent.y <= CoincollisionY1) {
                        this.bigAnimEntity.posCoin = { x: 1100, y: this.bigAnimEntity.posCoin.y };
                        this.score += 1;
                    }
                }
                if (babycollisionX >= Coin1collisionX && babycollisionX <= Coin1collisionX1) {

                    if (this.babyAnimEntity.posCurrent.y >= Coin1collisionY && this.babyAnimEntity.posCurrent.y <= Coin1collisionY1) {
                        this.bigAnimEntity.posCoin1 = { x: 1400, y: this.bigAnimEntity.posCoin1.y };
                        this.score += 1;
                    }
                }
                if (babycollisionX >= Coin2collisionX && babycollisionX <= Coin2collisionX1) {

                    if (this.babyAnimEntity.posCurrent.y >= Coin2collisionY && this.babyAnimEntity.posCurrent.y <= Coin2collisionY1) {
                        this.bigAnimEntity.posCoin2 = { x: 1400, y: this.bigAnimEntity.posCoin2.y };
                        this.score += 1;
                    }
                }
            ...
            checkTap: function () {
                ...
                if (this.pointer.isFirstPressed) {
                    ...
                    if (this.isGameOver == 0) {
                        ...
                        this.gameOverEntity.InstallButtonClick();
                    }
            ...
            secondTap: null,
            lastPlayerPos: null,
            ThreeTimeTap: function () {
                this.babyAnimEntity.tapcount += 1;
                if (this.babyAnimEntity.tapcount <= 3) {
                    if (this.babyAnimEntity.tapcount == 1) {
                        this.lastPlayerPos = this.babyAnimEntity.posCurrent.y;
                        this.babyAnimEntity.JumpBabyDragon(100);
                        console.log("tap01");
                    }
                    else if (this.babyAnimEntity.tapcount == 2) {
                        //console.log(Math.abs(this.lastPlayerPos - this.babyAnimEntity.posCurrent.y));
                        if (this.babyAnimEntity.posCurrent.y < this.lastPlayerPos) {
                            this.lastPlayerPos = this.babyAnimEntity.posCurrent.y;
                            this.babyAnimEntity.JumpBabyDragon(150);
                            console.log("tap02");
                        } else {
                            this.lastPlayerPos = this.babyAnimEntity.posCurrent.y;
                            this.babyAnimEntity.JumpBabyDragon(100);
                    ...
                    else if (this.babyAnimEntity.tapcount == 3) {
                        if (this.babyAnimEntity.posCurrent.y < this.lastPlayerPos) {
                            this.babyAnimEntity.JumpBabyDragon(150);
                            this.babyAnimEntity.tapcount = 0;
                            console.log("tap03");
                        } else {
                            this.babyAnimEntity.JumpBabyDragon(50);
                            this.babyAnimEntity.tapcount = 0;
                        }
